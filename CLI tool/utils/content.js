...
const generateControllerContent = (args) => {
  ...
  const create${capitalize(fileName)} = catchAsync(async (req, res) => {
    ...
    res
      ...
      .send({ success: true, message: '${capitalize(fileName)} created successfully' });
  });
  ...
  const get${capitalize(fileName)}ById = catchAsync(async (req, res) => {
    const ${fileName} = await ${fileName}Service.get${capitalize(fileName)}ById(req.params.id);
    if (!${fileName}) {
      return res.status(httpStatus.NOT_FOUND).send({ message: '${capitalize(fileName)} not found' });
    }
    res.status(httpStatus.OK).json(${fileName});
  });

  const update${capitalize(fileName)} = catchAsync(async (req, res) => {
    await ${fileName}Service.update${capitalize(fileName)}(req.params.id, req.body);
    res
      .status(httpStatus.OK)
      .send({ success: true, message: '${capitalize(fileName)} updated successfully' });
  });
  ...
  const delete${capitalize(fileName)} = catchAsync(async (req, res) => {
    ...
    res
      .status(httpStatus.OK)
      .send({ success: true, message: '${capitalize(fileName)} deleted successfully' });
  });
...
const generateRouteContent = (args) => {
  ...
  router.get('/${fileNameLowerPlural}', ${fileName}Controller.get${capitalize(fileNamePlural)});
...
//[Export]
const generateExportStatement = (fileName, schematicName) => {
  return `\nmodule.exports.${fileName}${capitalize(schematicName)} = require('./${fileName}.${schematicName}');`;
};
...
const generateModelContent = (args) => {
  ...
  if (fields.length !== types.length) throw new Error("Field length should equal to types length");

  const modelNameInCapital = capitalize(modelName);
  const schemaFields = fields.map((fieldName, index) => `${fieldName}: { type: ${getType(types[index])} }`).join(",\n    ");
  ...
  module.exports = ${modelNameInCapital};`.replaceAll("\n  ", "\n");
};
...
const generateValidationContent = (args) => {
  ...
  const createSchemaFields = fields.map((fieldName, index) => `${fieldName}: ${getJoiType(types[index])}`).join(",\n      ");
  const updateSchemaFields = fields.map((fieldName, index) => `${fieldName}: ${getJoiType(types[index])}.optional()`).join(",\n      ");
